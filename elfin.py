...
class elfin:
    ...
    def MoveRelL(self, directionID, distance):
        """
        Function: Robot moves a certain distance from the specified spatial coordinate direction
        Format: MoveRelL,rbtID, direction ID,direction,distance,;
        :param: direction:[X,Y,Z,RX,RY,RZ]
               distance: float
        :return:
        """
        direction = sign(int(distance))
        if direction == -1:
            direction = 0
        direction = str(direction)
        directionID = str(directionID)
        distance = str(abs(distance))
        message = "MoveRelL," + self.rbtID + ',' + directionID + ',' + direction + ',' + distance + self.end_msg
        print(message)
        return self.send(message)
